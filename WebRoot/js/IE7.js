...
(function() {
    ...
    if (/ie7_off/.test(top.location.search) || k < 5) return;
    var Q = bG();
    ...
    if (k < 5.5) {
        ...
        G += "address,blockquote,body,dd,div,dt,fieldset,form," + "frame,frameset,h1,h2,h3,h4,h5,h6,iframe,noframes,object,p," + "hr,applet,center,dir,menu,pre,dl,li,ol,ul{display:block}"
    }
    ...
    var D = z.extend({
        ...
        exec: function(g) {
            var h = this,
            p = this[Z];
            return String(g).replace(new RegExp(this, this.ignoreCase ? "gi": "g"),
            function() {
                var a, b = 1,
                c = 0;
                while ((a = h[bo + p[c++]])) {
                    ...
                    if (arguments[b]) {
                        ...
                        switch (typeof f) {
                        case "function":
                            return f.apply(h, bC.call(arguments, b, d));
                        case "number":
        ...
        add: function(a, b) {
            ...
            if (!this[bo + a]) this[Z].push(String(a));
    ...
    function cv(a, b) {
        if (!b) b = a;
        var c = {};
        for (var d in a) c[d] = b[d];
        return c
    };
    function i(c) {
        var d = arguments;
        var f = new RegExp("%([1-" + arguments.length + "])", "g");
        return String(c).replace(f,
        function(a, b) {
    ...
    function bJ(c) {
        return "\x01" + bp.push(c.replace(cz,
        function(a, b) {
    ...
    function R(a, b, c) {
        ...
        if (c) a.elements[b.uniqueID] = b;
        else delete a.elements[b.uniqueID];
        return c
    };
    w(window, "onbeforeprint",
    function() {
        if (!IE7.CSS.print) new bw("print");
        IE7.CSS.print.recalc()
    });
    var bL = /^\d+(px)?$/i;
    var J = /^\d+%$/;
    var E = function(a, b) {
        if (bL.test(b)) return parseInt(b);
        var c = a.style.left;
    ...
    IE7.recalc = function() {
        IE7.HTML.recalc();
        IE7.CSS.recalc();
        for (var a = 0; a < bs.length; a++) bs[a]()
    };
    function bc(a) {
    ...
    IE7._f = function(a, b) {
        ...
        var c = bd ? (a.attributes[b] || a.attributes[bu[b.toLowerCase()]]) : a.getAttributeNode(b);
    ...
    var bR = D.extend({
        ...
        format: function(a) {
            return a.replace(cI, "$1").replace(cH, "$1 $2").replace(cG, "$1*$2")
        },
        optimise: function(a) {
    ...
    var T = new bR({
        ...
        "#([\\w-]+)": function(a, b) {
            o = false;
            var c = "if(e%1.id=='%2'){";
            if (x) c += i("i%1=n%1.length;", x);
            return i(c, m, b)
        },
        "\\.([\\w-]+)": function(a, b) {
            o = false;
            F.push(new RegExp("(^|\\s)" + bb(b) + "(\\s|$)"));
            return i("if(e%1.className&&reg[%2].test(e%1.className)){", m, F.length - 1)
        },
    ...
    IE7.CSS = new(bM.extend({
        ...
        recalc: function() {
            ...
            for (q = b; q < c; q++) {
                ...
                if (f && (g = r.match(a))) {
                    p = B(f.selectorText);
                    if (p.length) for (j = 0; j < g.length; j++) {
                        l = g[j];
                        h = IE7.CSS.recalcs[l.slice(10)][2];
                        for (u = 0; (t = p[u]); u++) {
                            if (t.currentStyle[l]) h(t, r)
                        }
                    }
                }
            }
    ...
    var bw = z.extend({
        ...
        getText: function() {
            ...
            function r(a) {
                if (t.test(a)) return "all";
                else if (q.test(a)) return (j.test(a)) ? "all": "screen";
                else if (j.test(a)) return "print"
            };
            var N = this;
            function O(a, b, c, d) {
                var f = "";
            ...
            function cg(a, b) {
                var c = W(a.href, b);
                if (bl[c]) return "";
                bl[c] = (a.disabled) ? "": ci(IE7.CSS.getText(a, b), bn(a.href, b));
                return bl[c]
            };
            var ch = /(url\s*\(\s*['"]?)([\w\.]+[^:\)]*['"]?\))/gi;
            function ci(a, b) {
                return a.replace(ch, "$1" + b.slice(0, b.lastIndexOf("/") + 1) + "$2")
            };
        ...
        recalc: function() {
            var a, b;
            for (b = 0; (a = this.rules[b]); b++) a.recalc()
        },
        toString: function() {
    ...
    var L = n.extend({
        ...
        recalc: function() {
            ...
            for (var c = 0; b = a[c]; c++) {
                ...
                if (d.length) this.dynamicPseudoClass.apply(b, d, this)
            }
        }
    });
    var cd = z.extend({
        ...
        register: function(a) {
            ...
            if (!this.instances[a.id]) {
                var c = a[1],
                d;
                for (d = 0; d < c.length; d++) b.add(c[d]);
        ...
        unregister: function(a) {
            if (this.instances[a.id]) {
                ...
                var c = a[1],
                d;
                for (d = 0; d < c.length; d++) b.remove(c[d]);
    ...
    if (k < 7) {
        ...
        function(a) {
            var b = arguments;
            IE7.CSS.addEventHandler(a, k < 5.5 ? "onmouseover": "onmouseenter",
            function() {
                U.register(b)
            });
        ...
        function() {
            var a = U.instances;
            for (var b in a) if (!a[b][0].contains(event.srcElement)) U.unregister(a[b])
        })
    }
    IE7.CSS.addRecalc("[\\w-]+", "inherit",
    function(c, d) {
        var f = d.match(/[\w-]+\s*:\s*inherit/g);
        for (var g = 0; g < f.length; g++) {
            var h = f[g].replace(/ie7\-|\s*:\s*inherit/g, "").replace(/\-([a-z])/g,
            function(a, b) {
    ...
    function(_) {
        ...
        function collapseMargin(a, b) {
            if (!a.runtimeStyle[b]) {
                ...
                if (c && IE7.hasLayout(c) && !IE7[b == "marginTop" ? "_b": "_a"](a)) return;
                var d = B(">*:" + (b == "marginTop" ? "first": "last") + "-child", a, true);
        ...
        var fixWidth = function(g) {
            applyWidth = function(a) {
                if (!J.test(a.currentStyle.width)) h(a);
                collapseMargins(a)
            };
            function h(a, b) {
                if (!a.runtimeStyle.fixedWidth) {
                    if (!b) b = a.currentStyle.width;
                    a.runtimeStyle.fixedWidth = (UNIT.test(b)) ? Math.max(0, q(a, b)) : b;
            ...
            function t(a, b) {
                if (J.test(b)) return parseInt(parseFloat(b) / 100 * p(a));
                return E(a, b)
            };
            var q = function(a, b) {
                var c = a.currentStyle["box-sizing"] == "border-box";
                var d = 0;
                if (C && !c) d += j(a) + u(a, "padding");
                else if (!C && c) d -= j(a) + u(a, "padding");
                return t(a, b) + d
            };
            function j(a) {
            ...
            function u(a, b) {
                return t(a, a.currentStyle[b + "Left"]) + t(a, a.currentStyle[b + "Right"])
            };
            ...
            function l(a) {
                ...
                if (a.currentStyle.minWidth != "none" && c <= q(a, a.currentStyle.minWidth)) {
                    a.runtimeStyle.width = a.currentStyle.minWidth
                } else if (a.currentStyle.maxWidth != "none" && c >= q(a, a.currentStyle.maxWidth)) {
            ...
            function r(a) {
                if (R(r, a, /^(fixed|absolute)$/.test(a.currentStyle.position) && bt(a, "left") != "auto" && bt(a, "right") != "auto" && AUTO.test(bt(a, "width")))) {
                    N(a);
            ...
            bq(function() {
                ...
                for (a in c) {
                    var d = c[a];
                    var f = (parseInt(d.runtimeStyle.width) == q(d, d.currentStyle.minWidth));
                    if (b && f) d.runtimeStyle.width = "";
                    if (b == f) l(d)
                }
                var c = layout.maxWidth.elements;
                for (a in c) {
                    var d = c[a];
                    var f = (parseInt(d.runtimeStyle.width) == q(d, d.currentStyle.maxWidth));
                    if (!b && f) d.runtimeStyle.width = "";
                    if (b != f) l(d)
                }
                for (a in r.elements) N(r.elements[a])
            });
            if (C) {
    ...
    if (k >= 5.5 && k < 7) {
        ...
        function(a, b, c, d, f) {
            d = cC(d);
            return bi.test(d) ? "filter:" + i(bx, d, "crop") + ";zoom:1;background" + (b || "") + ":" + (c || "") + "none" + (f || "") : a
        });
        IE7.HTML.addRecalc("img,input",
        function(a) {
            if (a.tagName == "INPUT" && a.type != "image") return;
            by(a);
            w(a, "onpropertychange",
            function() {
                if (!bj && event.propertyName == "src" && a.src.indexOf(bg) == -1) by(a)
            })
        });
        var bj = false;
        w(window, "onbeforeprint",
        function() {
            bj = true;
            for (var a = 0; a < M.length; a++) ce(M[a])
        });
        w(window, "onafterprint",
        function() {
            for (var a = 0; a < M.length; a++) bz(M[a]);
            bj = false
        })
    }
    function bz(a, b) {
    ...
    function(_) {
        ...
        function _2(a) {
            if (R(_2, a, a.currentStyle.backgroundAttachment == "fixed" && !a.contains(v))) {
                _3();
                bgLeft(a);
                bgTop(a);
                _8(a)
        ...
        function getOffsetLeft(a, b) {
            ...
            default:
                if (J.test(b)) {
                    return parseInt((s.clientWidth - _0.offsetWidth) * parseFloat(b) / 100)
                }
                _0.style.left = b;
        ...
        function _4(a, b) {
            ...
            if (!a.runtimeStyle.autoLeft && a.currentStyle.marginLeft == "auto" && a.currentStyle.right != "auto") {
                var c = s.clientWidth - getPixelWidth(a, a.currentStyle.right) - getPixelWidth(a, a.runtimeStyle._c) - a.clientWidth;
                if (a.currentStyle.marginRight == "auto") c = parseInt(c / 2);
                if (_1(a.offsetParent)) a.runtimeStyle.pixelLeft += c;
                else a.runtimeStyle.shiftLeft = c
            }
            clipWidth(a);
            clipHeight(a)
        };
        ...
        function getScreenLeft(a) {
            ...
            if (a.runtimeStyle.autoLeft) {
                b = s.clientWidth - a.offsetWidth - getPixelWidth(a, a.currentStyle.right)
            }
        ...
        function getPixelWidth(a, b) {
            return J.test(b) ? parseInt(parseFloat(b) / 100 * s.clientWidth) : E(a, b)
        };
        eval(A(getPixelWidth));
        function _g() {
            var a = _2.elements;
            for (var b in a) _8(a[b]);
            a = _6.elements;
            for (b in a) {
                _4(a[b], true);
                _4(a[b], true)
    ...
    IE7.CSS.addRecalc("overflow", "visible",
    function(a) {
    ...
    function cf() {
        ...
        IE7.CSS.addFix(/(font(-size)?\s*:\s*)([\w.-]+)/,
        function(a, b, c, d) {
            return b + (f[d] || d)
        });
        if (k < 6) {
            ...
            q = /ex$/i;
            E = function(a, b) {
                if (bL.test(b)) return parseInt(b) || 0;
                var c = h.test(b) ? -1 : 1;
                if (p.test(b)) c *= u(a);
            ...
            function u(a) {
                ...
                while (a != v) {
                    ...
                    if (c) {
                        if (t.test(c)) b *= parseFloat(c);
                        else if (J.test(c)) b *= (parseFloat(c) / 100);
                        else if (q.test(c)) b *= (parseFloat(c) / 2);
                        else {
                            j.style.fontSize = c;
                            return 1
                        }
        ...
        function l(a) {
            if (k < 5.5) IE7.Layout.boxSizing(a.parentElement);
        ...
        IE7.CSS.addRecalc("margin(-left|-right)?", "[^};]*auto",
        function(a) {
            if (R(l, a, a.parentElement && a.currentStyle.display == "block" && a.currentStyle.marginLeft == "auto" && a.currentStyle.position != "absolute")) {
                l(a)
            }
        });
        bq(function() {
    ...
    IE7.loaded = true; (function() {
        ...
        bi = new RegExp(bb(typeof IE7_PNG_SUFFIX == "string" ? IE7_PNG_SUFFIX: "-trans.png") + "$", "i");
        v = document.body;
        s = C ? v: bm;
